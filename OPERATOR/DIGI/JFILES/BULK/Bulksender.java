...
import org.apache.activemq.command.ActiveMQTextMessage;
	...
	public class Bulksender extends Thread{
	...
	public void run()
	{
		...
		while(true)
		{
			try
			{
				String qquery = "select billing_ID,msisdn,event_type,amount,service_id,operator,circle,plan_id,MODE from tbl_billing_reqs nolock where status=5 and (date(now()) not in ('2013-11-07') or hour(now()) not in(0,1,2,3,4,5,6))  limit 0,1000";
				qquery=qquery.trim();
				//System.out.println(qquery);
				Thread.sleep(1000);
				ResultSet rs= stmt.executeQuery(qquery);
...
//========================================================
	...
	public static void main(String args[])
	{
                try
                {
                        int status=checkProcess.CheckProcess.GetProcessList("Bulksender",1);
                        if(status>1)
                        {
