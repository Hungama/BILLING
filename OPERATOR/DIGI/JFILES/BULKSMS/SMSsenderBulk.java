...
public class SMSsenderBulk extends Thread
{
...
//****************************************MAIN FUNCTION ********************************************************
	public static void main(String args[])
	{
                try
                {
                        int status=checkProcess.CheckProcess.GetProcessList("SMSsenderBulk",1);
                        if(status>1)
                        {
		...
		SMSsenderBulk SMSSENDER = new SMSsenderBulk();
		SMSSENDER.start();
	}
...
//		    check_multiple();
			...
			try
			{
				...
				System.out.println("BULKSMS Queue Connetion");
				//=================================================
		    ...
		    while(true)
		    {
		    	try
		    	{
					...
					int msgid=0,status;
					stmtBGM.executeUpdate("delete from tbl_bulk_sms where message is null");
					stmtBGM.executeUpdate("delete from tbl_bulk_sms where message =''");
					 STR = "select msgid,ani,message,date_time,dnis,amount,type,status from  tbl_bulk_sms where date(date_time)>=date(subdate(now(),1)) and status=0 and message is not null limit 1000";
	                ...
	                while(rs .next())
					{
						...
						amount=rs.getString("amount").trim();msg_type=rs.getString("type");
						status = rs.getInt("status");

						System.out.println("Picked Mobile :"+ani+" message "+orignal_msg+ " amount "+amount);

						log("SMSSenderBulk#"+msgid+"#"+ani+"#"+orignal_msg+"#"+date_time+"#"+dnis+"#"+msg_type+"DIGM#DIG#"+status+"#NoDND");

						messageQ.setText(msgid+"#"+ani+"#"+message+"#"+date_time+"#"+dnis+"#"+amount+"#"+msg_type);
						producer.send(messageQ,2,9,10000000);
						stmtBGM.executeUpdate("delete from tbl_bulk_sms where msgid='"+msgid+"'");
						sleep(30);
