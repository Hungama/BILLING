		...
		try
			{
				...
				cnt=1;
				String STR;
				if("docomo_radio.tbl_radio_subscription".equalsIgnoreCase(SUBSCRIPTION_TABLE[server]))
			{
Date gsk=new Date();
if(gsk.getHours()==20)
{
...
}				else if("indicom_radio.tbl_radio_subscription".equalsIgnoreCase(SUBSCRIPTION_TABLE[server]))
					STR = "select ani,datediff(now(),renew_date) as 'dayleft' from "+SUBSCRIPTION_TABLE[server]+" nolock where date(renew_date)<=date(now()) and status in(1,11) and mode_of_sub in ('TOBD','TIVR','TUSSD')";
				else
					STR = "select ani,datediff(now(),renew_date) as 'dayleft' from "+SUBSCRIPTION_TABLE[server]+" nolock where date(renew_date)<=date(now()) and status in(1,11)";
				ResultSet rsMobile = stmtFM.executeQuery(STR);
				System.out.println(STR);
