...
import org.apache.activemq.command.ActiveMQTextMessage;
	...
	public class RESUBsender2 extends Thread{
	...
	//	while(true)
		{
			try
			{
				String qquery = "select billing_ID,msisdn,event_type,amount,service_id,operator,circle,plan_id,MODE from tbl_billing_reqs nolock where event_type='RESUB' and status in(111,112,113,114,115)";//and msisdn in('8699509743','9324963670')
				qquery=qquery.trim();
				//System.out.println(qquery);
				Thread.sleep(10);
				ResultSet rs= stmt.executeQuery(qquery);
						...
						try
						{
							...
							//	message.setText(out_string);
								//producer.send(message);
                                                                System.out.println("Queue_Pre - "+new java.util.Date().getTime()+"   "+i++);
							...
							//	hunLog(out_string,'s');
								...
								//stmtUpdate.executeUpdate("update tbl_billing_reqs set status=2 where billing_ID='"+billing_ID+"'");
								 System.out.println("Pre-Post - "+new java.util.Date().getTime());
							}
							else
							{
...
//========================================================
	...
	public static void main(String args[])
	{
		...
		RESUBsender2 sBM = new RESUBsender2();
		sBM.readBillType();
		sBM.start();
	}
