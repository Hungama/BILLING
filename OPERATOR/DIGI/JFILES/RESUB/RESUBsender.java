...
import org.apache.activemq.command.ActiveMQTextMessage;
	...
	public class RESUBsender extends Thread{
	...
	public void run()
	{
		...
		while(true)
		{
			try
			{
				String qquery = "select billing_ID,msisdn,event_type,amount,service_id,operator,circle,plan_id,MODE from tbl_billing_reqs nolock where event_type='RESUB' and status=0 limit 0,500";//and msisdn in('8699509743','9324963670')
				qquery=qquery.trim();
				//System.out.println(qquery);
				Thread.sleep(10);
				ResultSet rs= stmt.executeQuery(qquery);
...
//========================================================
	...
	public static void main(String args[])
	{
                try
                {
                        int status=checkProcess.CheckProcess.GetProcessList("RESUBsender",1);
                        if(status>1)
                        {
		...
		RESUBsender sBM = new RESUBsender();
		sBM.readBillType();
		sBM.start();
	}
}
