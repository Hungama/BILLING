	...
	public String ValidateAndBill(String ani,String pricecode,String mode,String kiword,String cpa_sid)
	{
		...
		try
		{
		  ...
		  //============================================================
			...
			System.out.println("Starttime-"+starttime+"#endtime-"+endtime+"#for ani is"+ani);
			writeXML(ani,str);
...
public  void writeXML(String ani,String xmlstr)
	{
		try
		{
			...
			outprint.print("#"+mystrdate+"#"+mystrtime+"#"+xmlstr);
			outprint.close();
			outfile.close();
...
{
	String transid=null,error_code=null,error_desc=null,error_list="NA",success_list="NA";
	try
	{
		transid   = StringUtils.trim(StringUtils.substringBetween(str,"<transaction_id>", "</transaction_id>"));
		error_code= StringUtils.trim(StringUtils.substringBetween(str,"<error_code>", "</error_code>"));
		error_desc= StringUtils.trim(StringUtils.substringBetween(str,"<error_desc>", "</error_desc>"));

		if(error_code.equals("1"))
		{
			error_list="NA";
			success_list=StringUtils.trim(StringUtils.substringBetween(str,"<success_list>", "</success_list>"));
		}
		else
		{
			error_list=StringUtils.trim(StringUtils.substringBetween(str,"<error_list>", "</error_list>"));
			if(error_list!=null || !error_list.equals("") )
			{
				if(error_list.indexOf("%3A")>-1)
				{
					error_list=error_list.substring(error_list.indexOf("%3A")+3,error_list.length());
					error_list=error_list.substring(0,error_list.indexOf("%2C"));
				}
