...
import org.apache.activemq.command.ActiveMQTextMessage;
	...
	public class DIGIsender extends Thread{
	...
	public void run()
	{
		...
		while(true)
		{
			try
			{
				String qquery = "select billing_ID,msisdn,event_type,amount,service_id,operator,circle,plan_id,MODE from tbl_billing_reqs nolock where event_type in ('SUB','TOPUP') and status=0  and (date(now()) not in ('2013-11-07') or hour(now()) not in(0,1,2,3,4,5,6)) limit 0,500";//and msisdn in('8699509743','9324963670')
				qquery=qquery.trim();
				//System.out.println(qquery);
				Thread.sleep(1000);
				ResultSet rs= stmt.executeQuery(qquery);
				if(rs!=null)
				{
					while(rs.next())
					{
						try
						{
							...
							{
								out_string = billing_ID+"#"+msisdn+"#"+event_type+"#"+amount+"#"+service_id+"#"+operator+"#"+circle+"#"+plan_id+"#"+hashMap.get(key)+"#"+in_mode;
...
public void hunLog(String log,char file)
	{
		...
		try
		{
			...
			case's':
				System.out.println("Under case S  --------> ");
...
//========================================================
	...
	public static void main(String args[])
	{
		try
		{
			int status=checkProcess.CheckProcess.GetProcessList("DIGIsender",1);
			if(status>1)
			{
	...
	// System.out.println("Process is in running state");
	// System.exit(1);
		try
		{
			//check_multiple();
