...
import org.apache.activemq.command.ActiveMQTextMessage;
	...
	public class VODMsender extends Thread{
	...
	public void run()
	{
		...
		while(true)
		{
			try
{
//String qquery = "select billing_ID,msisdn,event_type,amount,service_id,operator,circle,plan_id,MODE,subservice_id from tbl_billing_reqs nolock where status=0 and operator in('VODM') and (date(now())) limit 0,2000";
//
//String qquery="select billing_ID,msisdn,event_type,amount,service_id,operator,circle,plan_id,MODE,subservice_id from tbl_billing_reqs nolock where status=0 and operator in('VODM')  and circle not in ('MUM','MAH') limit 0,2000";

String qquery = "select billing_ID,msisdn,event_type,amount,service_id,operator,circle,plan_id,MODE,subservice_id from tbl_billing_reqs nolock where status=0 and operator in('VODM')  and (date(now()) not in ('2013-06-19') or hour(now()) not in(0,1,2,3,4,5)) limit 0,2000";
				qquery=qquery.trim();
				//System.out.println(qquery);
				Thread.sleep(1000);
				ResultSet rs= stmt.executeQuery(qquery);
				if(rs!=null)
				{
					while(rs.next())
					{
						try
						{
							...
							String subservice_id = rs.getString("subservice_id");
							String out_string = null;
							String key=plan_id+"-"+service_id;
							if(hashMap.get(key)!=null)
							{
								out_string = billing_ID+"#"+msisdn+"#"+event_type+"#"+amount+"#"+service_id+"#"+operator+"#"+circle+"#"+plan_id+"#"+hashMap.get(key)+"#"+in_mode+"#"+subservice_id;
		...
		try
		{
			...
			case's':
				...
				else
				{
					...
					appender = new RollingFileAppender(new PatternLayout(),Path+"Sender_"+strdate+".log");
