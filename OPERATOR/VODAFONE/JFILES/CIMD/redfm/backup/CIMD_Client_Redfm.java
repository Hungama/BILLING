		...
		try
		{
			...
			while(true)
			{
				...
				try
				{
						...
						int cnt=0;
						ResultSet rs1=stmtcnt.executeQuery(str);
 						while(rs1.next())
						  {
							cnt= rs1.getInt("cnt");
						  }
						 if(cnt>0)
                         {
							 	String qquery = "select msgid,ani,message,date_time,dnis,flag from tbl_sms nolock where status=0  and message is not null  and flag=2 limit  0,500";
							        qquery=qquery.trim();
							...
							else
							{
								...
								try
								{
									...
									{
										int retcode = IsAlive();
										cimd_functions.PrintAndLog("CIMD IsAlive() response "+retcode);
										if(retcode<1)
										{
