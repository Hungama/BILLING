...
public class EtislatSenderTF extends Thread
{
...
//****************************************MAIN FUNCTION ********************************************************
	...
	{
		EtislatSenderTF SMSSENDER = new EtislatSenderTF();
		SMSSENDER.start();
	}
		...
		try
		{
		    ...
		    while(true)
		    {
		    	...
		    	try
		    	{
					...
					int msgid=0;
					STR = "select msg_id,ani,message,date_time,dnis,type from etislat_hsep.tbl_sms nolock where status=0 and message is not null and length(message)<>0 order by temp desc limit 200 ";
