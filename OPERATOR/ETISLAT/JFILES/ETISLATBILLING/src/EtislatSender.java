...
import org.apache.activemq.command.ActiveMQTextMessage;
	...
	public class EtislatSender extends Thread
	{
		...
		private static String subject = "",subjectQ="";//new
		...
		String dsmlogs="/home/ivr/javalogs/Etislat/",msgqueue=null,msgsubqueue=null;
		String qquery=null;
		...
		public EtislatSender()
		{
			try
			{
				...
				msgsubqueue=resource.getString("MSGSUBQUEUE");
			    System.out.println("IP: "+ip+" DATABASE :"+dsn+" USER :"+username+" PWD:"+pwd);
			    subject = msgqueue;
			    subjectQ = msgsubqueue;//new
			}
			catch(Exception e)
			{
	...
	public void run()
	{
		...
		try
		{
			...
			ConnectionFactory connectionFactoryQ = new ActiveMQConnectionFactory(url);
		    connectionQ = connectionFactoryQ.createConnection();
		    connectionQ.start();
		    sessionQ = connectionQ.createSession(false,Session.AUTO_ACKNOWLEDGE);
	        destinationQ = sessionQ.createQueue(subjectQ);
	        producerQ = sessionQ.createProducer(destinationQ);
	        System.out.println(msgsubqueue+" Active message Queue established!");
		...
		try
		{
			...
			while(true)
			{
				...
				if(counter%2==0)
				{
					qquery = "select billing_ID,msisdn,event_type,amount,service_id,operator,circle,plan_id,MODE,subservice_id,sc from master_db.tbl_billing_reqs nolock where status=0 and service_id='2121'  and event_type in ('SUB') limit 0,500";
				}
				else
				{
					qquery = "select billing_ID,msisdn,event_type,amount,service_id,operator,circle,plan_id,MODE,subservice_id,sc from master_db.tbl_billing_reqs nolock where status=0 and service_id='2121'  and event_type in ('RESUB') limit 0,500";
				}
				 qquery=qquery.trim();
				 Thread.sleep(1000);
				 ResultSet rs= stmt.executeQuery(qquery);
						...
						try
						{
							...
							if(hashMap.get(key)!=null)
							{
								...
								_priority=9;
							 	...
							 	{
							 		messageQ.setText(out_string);
								 	producerQ.send(messageQ,2,_priority,100000000);//2=PERSISTENT,1=NON PERSISTENT
							 	}
